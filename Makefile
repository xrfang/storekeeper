...
comprel:
	mkdir -p bin && cd src && CGO_CFLAGS=$(CGO_CFLAGS) go build -ldflags="-s -w" . && mv $(GOMOD)* ../bin
compdbg:
	mkdir -p bin && cd src && CGO_CFLAGS=$(CGO_CFLAGS) go build -race -gcflags=all=-d=checkptr=0 . && mv $(GOMOD)* ../bin
pack: export GOOS=
pack: export GOARCH=
pack: export GOARM=
pack:
